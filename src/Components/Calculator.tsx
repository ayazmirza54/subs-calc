...
const SubscriptionCalculator = () => {
  ...
  const [customBilling, setCustomBilling] = useState('monthly');
  ...
  const addSubscription = (sub, billingType = 'monthly') => {
    const newSub = {
      id: Date.now(),
      name: sub.name,
  ...
  const totalMonthly = subscriptions.reduce((sum, sub) => 
    sum + (sub.billingType === 'monthly' ? sub.monthlyPrice : sub.yearlyPrice / 12), 0
  );

  const totalYearly = subscriptions.reduce((sum, sub) => 
    sum + (sub.billingType === 'yearly' ? sub.yearlyPrice : sub.monthlyPrice * 12), 0
  );
          ...
          <div className={`rounded-xl shadow-lg p-6 transition-colors ${
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {popularSubscriptions.ai.map((sub, index) => (
                <div key={index} className={`flex items-center justify-between p-3 rounded-lg hover:opacity-80 transition-all ${
              ...
              <>
                <div className="space-y-3 mb-6 max-h-64 overflow-y-auto">
                  {subscriptions.map((sub) => (
                    <div key={sub.id} className={`flex items-center justify-between p-3 rounded-lg transition-colors ${
